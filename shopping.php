...
$items = array();
$query = "SELECT * FROM GameCatalog ORDER BY ID DESC";
...
header('Location: shopPage.php'); //Add this line. We specify index.php.
exit();
}
...
if (isset($_POST['action']) and $_POST['action'] == 'Checkout')
{
    foreach ($_SESSION['cart'] as $id) {
        foreach ($items as $product) {
            if ($product['id'] == $id){
                $query = "UPDATE GameCatalog SET Sales = Sales + 1 WHERE ID = $id";
                $result = $conn->query($query);
            }
